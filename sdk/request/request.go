...
func (r *Request) Send2(retries int, ignvdom bool, headers *map[string]string) error {
	...
	for {
		...
		if errdo != nil {
			...
			slog.Error("Error found: %v, will resend again %s, %d", slog.Any("error_message", filterapikey(errdo.Error())), slog.Any("url", u), slog.Any("retry_count", retry))
	...
	if login_method == "session" {
		...
		if errLogout != nil {
			slog.Warn("Issue occurs when logout session", slog.Any("error", errLogout))
		}
	} else if login_method == "token" {
		errLogout := r.LogoutToken(token, cookies)
		if errLogout != nil {
			slog.Warn("Issue occurs when logout token", slog.Any("error", errLogout))
		}
	}
...
func (r *Request) Send3(vdomparam string, headers *map[string]string) error {
	...
	slog.Debug("Logged in successfully - Send3 URL", slog.Any("url", u), slog.Any("headers", r.HTTPRequest.Header))
	retry := 0
	for {
		//Send
		rsp, errdo := r.Config.HTTPCon.Do(r.HTTPRequest)
