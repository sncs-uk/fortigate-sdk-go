...
// sortFirewallProxypolicyItem contains the parameters for each Policy item
...
func getPolicyListFirewallProxypolicy(c *FortiSDKClient, vdomparam string) (itemList []sortFirewallProxypolicyItem, err error) {
	HTTPMethod := "GET"
...
func bPolicyListSortedFirewallProxypolicy(itemList []sortFirewallProxypolicyItem, sortby, sortdirection string, manual_order []string) (bsorted bool) {
	bsorted = true
	if sortby == "policyid" {
...
func sortPolicyListFirewallProxypolicy(itemList []sortFirewallProxypolicyItem, sortby, sortdirection string, c *FortiSDKClient, vdomparam string, manual_order []string) (err error) {
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallProxypolicy(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, vdomparam)
		if err != nil {
...
func (c *FortiSDKClient) CreateUpdateFirewallProxypolicySort(sortby, sortdirection, vdomparam string, manual_order []string) (err error) {
	itemList, err := getPolicyListFirewallProxypolicy(c, vdomparam)
	...
	bsorted := bPolicyListSortedFirewallProxypolicy(itemList, sortby, sortdirection, manual_order)
	if bsorted == true {
		return
	}

	err = sortPolicyListFirewallProxypolicy(itemList, sortby, sortdirection, c, vdomparam, manual_order)
	if err != nil {
		err = fmt.Errorf("sort err %s", err)
		return
	}
...
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallProxypolicySort(sortby, sortdirection string, vdomparam string, manual_order []string) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getPolicyListFirewallProxypolicy(c, vdomparam)
	if err != nil {
		err = fmt.Errorf("sort err %s", err)
		return
