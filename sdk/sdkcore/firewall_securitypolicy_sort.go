...
// sortFirewallSecuritypolicyItem contains the parameters for each Policy item
...
func getPolicyListFirewallSecuritypolicy(c *FortiSDKClient, vdomparam string) (itemList []sortFirewallSecuritypolicyItem, err error) {
	HTTPMethod := "GET"
...
func bPolicyListSortedFirewallSecuritypolicy(itemList []sortFirewallSecuritypolicyItem, sortby, sortdirection string, manual_order []string) (bsorted bool) {
	bsorted = true
	if sortby == "policyid" {
...
func moveAfterFirewallSecuritypolicy(idbefore, idafter int, c *FortiSDKClient, vdomparam string) (err error) {
...
func sortPolicyListFirewallSecuritypolicy(itemList []sortFirewallSecuritypolicyItem, sortby, sortdirection string, c *FortiSDKClient, vdomparam string, manual_order []string) (err error) {
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallSecuritypolicy(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, vdomparam)
		if err != nil {
...
func (c *FortiSDKClient) CreateUpdateFirewallSecuritypolicySort(sortby, sortdirection, vdomparam string, manual_order []string) (err error) {
	itemList, err := getPolicyListFirewallSecuritypolicy(c, vdomparam)
	...
	bsorted := bPolicyListSortedFirewallSecuritypolicy(itemList, sortby, sortdirection, manual_order)
	if bsorted == true {
		return
	}

	err = sortPolicyListFirewallSecuritypolicy(itemList, sortby, sortdirection, c, vdomparam, manual_order)
	if err != nil {
		err = fmt.Errorf("sort err %s", err)
		return
	}
...
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallSecuritypolicySort(sortby, sortdirection string, vdomparam string, manual_order []string) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getPolicyListFirewallSecuritypolicy(c, vdomparam)
	if err != nil {
		err = fmt.Errorf("sort err %s", err)
		return
